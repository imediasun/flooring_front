...
export const SelectField = forwardRef(
    ({ value = "", name, holder, items, onChangeForm, ...props }, ref) => {
        ...
        const onOptionClickHandle = (newValue) => {
            close();
            onChangeForm && onChangeForm(name, newValue); // Используем переданное name
        };
        ...
        return (
            ...
            >
                <input type="hidden" value={value} name={name} ref={ref} {...props} />
                <button
                ...
                <ul className="form-field__select-list">
                    {items.map((item) => (
                        <li
                            key={item}
                            className={`form-field__select-list-item ${
                                item === value ? "_active" : ""
                            }`}
                        >
                            <button
